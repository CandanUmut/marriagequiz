...
export default function AlignmentBar({ alignment }: AlignmentBarProps) {
  ...
  return (
    <div className="rounded-xl border border-sand-200 dark:border-sand-800 bg-white dark:bg-sand-950 p-4">
      {/* Header row */}
      ...
      {/* Deal-breaker detail */}
      {hasDealBreaker && (
        <div className="mt-3 pt-2 border-t border-sand-100 dark:border-sand-800">
          <p className="text-xs text-sand-500 dark:text-sand-400">
            {alignment.personADealBreaker && alignment.personBDealBreaker
              ? (locale === 'en'
                ? 'Both partners flagged this as a deal-breaker.'
                : 'Her iki partner de bunu kırmızı çizgi olarak işaretledi.')
